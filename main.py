...
def main():
    ...
    if response in ['create', 'c', 'new']:
        ...
        if pkformat >= 16:
            print(
                'Add other supported versions? Y/N (old versions may not have full support)')
            yn = input('> ').lower()
            if yn in ['y', 'yes']:
        ...
        if yn in ['y', 'yes']:
            print(
                f'This hasn\'t been implemented yet, but you can manually add "pack.png" to Projects/{name}')
        else:
            ...
            if action in ['recipe', 'crafting recipe']:
                if pkformat < 48:
                    if not os.path.exists(
                            f'Projects/{name}/data/{ns}/recipes'):
                        makedirs(f'Projects/{name}/data/{ns}/recipes', exist_ok=True)
                else:
                    if not os.path.exists(f'Projects/{name}/data/{ns}/recipe'):
                        makedirs(f'Projects/{name}/data/{ns}/recipe', exist_ok=True)
                print('OK, making a new recipe')
                print(
                    'Crafting, stonecutting, smithing, or smelting (furnace/campfire/smoker/'
                    'blast furnace)?')
        ...
        if os.path.exists(f'Projects/{delname}'):
            ...
            print(
                'This project doesn\'t exist! Make sure the capitalization is correct next time.')
            main()
    elif response == 'exit':
        print('Exiting...')
        exit()
